$(function() {
	var draw = $('#draw'), context = draw[0].getContext('2d');
	...
	var cellFactory = {
		...
		create : function(point, color) {
			...
			context.fillRect(this.point.x, this.point.y, this.borderWidth,
					this.borderWidth);
		}
	}, snakeFactory = {
		points : [ {
		...
		init : function() {
			...
			speed = 1000, self.direction = 'left', self.points = [ {
				x : 250,
				y : 150
			}, {
				x : 260,
		...
		isOver : function(targetPoint) {
			...
			// 判断是否出界了
			var isChujie = (targetPoint.x < 0 || targetPoint.y < 0
					|| targetPoint.x > context.canvas.width - 10 || targetPoint.y > context.canvas.height - 10);
			return isYaodaoziji;
		},
		gameOver : function() {
			this.isRun = false;
		...
		run : function() {
			var self = this, first = self.points[0], targetPoint = tp = {
				x : 0,
				y : 0
			};
			...
			self.points = self.points.map(function(point, index) {
				...
				if (index == 0) {
					...
					targetPoint.x = targetPoint.x < 0 ? self.scpoe.x - 10
							: targetPoint.x;
					targetPoint.y = targetPoint.y < 0 ? self.scpoe.y - 10
							: targetPoint.y;
					targetPoint.x = targetPoint.x >= self.scpoe.x ? 0
							: targetPoint.x;
					targetPoint.y = targetPoint.y >= self.scpoe.y ? 0
							: targetPoint.y;
		...
		isEat : function() {
			return this.points[0].x == this.fruitPoint.x
					&& this.points[0].y == this.fruitPoint.y
		},
		eat : function() {
		...
		create : function(points) {
			...
			do {
				x = Math
						.floor((context.canvas.width - 10) / 10 * Math.random()) * 10;
				y = Math.floor((context.canvas.height - 10) / 10
						* Math.random()) * 10;
		...
		init : function() {
			...
			db
					.getAll(function(data) {
						...
						// 分数去重
						var unionScoreArray = _.union(self.scoreArray
								.map(function(obj) {
									return obj.score;
								}))

						// 获取排行榜最后一名的分数
						self.lastScore = unionScoreArray[self.last - 1 < unionScoreArray.length ? self.last - 1
								: unionScoreArray.length - 1] || 0;

						self.scoreArray = self.scoreArray.filter(function(obj) {
							return obj.score >= self.lastScore;
						});
						self.scoreArray.forEach(function(obj) {
							var tr = '<tr><td>' + obj.name + '</td><td>'
									+ obj.score + '</td></tr>';
		...
		add : function(score) {
			...
			if (score > 0 && score >= this.lastScore
					|| this.scoreArray.length < this.last) {
	...
	$(document).keydown(function(event) {
		if ((!snakeFactory.isRun) && event.keyCode != 32)
			return;
